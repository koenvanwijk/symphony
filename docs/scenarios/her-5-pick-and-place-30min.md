...
Dit scenario beschrijft hoe een medewerker van een metaal-MKB bedrijf een industriële robot
in maximaal 30 minuten kan instellen voor een standaard pick-and-place taak. De flow maakt
...
| Robotbereik | 600 – 1500 mm reikwijdte (industrieel 6-DOF arm) |
| Vloer/montagetype | Vloergemonteerd, vaste positie |
...
| Netwerk | Lokaal bedrijfsnetwerk (LAN), geen cloud-verplichting |
...
- Industriële 6-DOF robot arm (cobot, bijv. Universal Robots UR10e of gelijkwaardig)
- 3D-dieptecamera (bijv. Intel RealSense D435 of Azure Kinect DK) voor omgevingsscan
...
- Krachtsensor (optioneel, voor grijpbevestiging)
...
- **Padplanner**: kinematisch/collision-aware padplanningsalgoritme (bijv. MoveIt 2 of eigen)
- **Haalbaarheidsanalyser**: bereikbaarheids-, botsings- en lastanalyse
- **Teleop-interface**: joystick of webinterface voor handmatige correctie
- **Uitvoeringsruntime**: robot-middleware (ROS 2) voor commandoverzending
...
| 1:30 | Medewerker positioneert het robotmodel in de viewer op de gewenste montagepositie (drag-and-drop of coördinateninvoer) | Medewerker | Virtuele robot verschijnt in gescande omgeving op de juiste locatie |
...
| 9:00 | Systeem voert initiële last- en gripkrachtinschatting uit | Systeem | Grip-haalbaarheidsmelding (groen = haalbaar / geel = marginaal / rood = niet haalbaar) |
...
| # | Check | Wie | Methode |
|---|---|---|---|
| 1 | Werkcel vrij van personen | Medewerker | Visuele inspectie + bevestiging in UI |
...
| Tijd | Stap | Actor | Resultaat |
|---|---|---|---|
| 22:00 | Medewerker kiest "Begeleide testrun" (teleop-modus actief als vangnet) | Medewerker | Robot start approach naar pick-punt; teleop beschikbaar via joystick/UI |
| 23:00 | Robot voert pick-beweging uit; systeem toont real-time krachtsensorwaarde en gripstatus | Systeem | Grip bevestigd (kracht within tolerantie) of foutmelding |
...
- Teleop is beschikbaar als real-time correctiemiddel (joystick of webinterface)
...
| 29:00 | Systeem monitort continu: krachtsensor, plaatsnauwkeurigheid, cyclustijd | Systeem | Live dashboard zichtbaar |
...
2. **Robotmodel plaatsing**: drag-and-drop van UR10e-model in de viewer
...
5. **Padplanning**: MoveIt 2 of eenvoudige lineaire interpolatie met botsingscheck
...
8. **Teleop-interface**: joystick of pijltjestoetsen in webUI sturen virtuele robot
9. **Pre-executie checklist**: UI-wizard met 7 checkboxes
...
| Netwerk | LAN-verbinding robot ↔ werkstation; geen internetvereiste |
